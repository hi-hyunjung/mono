...
export function middleware(req: NextRequest) {
  ...
  if (!jwt && isProtected) {
    ...
    if (params.get('callback_url')) {
      return NextResponse.redirect(new URL(Path.SIGN_IN + '?callback_url=' + params.get('callback_url'), req.url));
    }
  } else {
    const requestPath = `${req.nextUrl.pathname}${encodeURIComponent(req.nextUrl.search)}`;
    return NextResponse.redirect(new URL(Path.SIGN_IN + '?callback_url' + requestPath, req.url));
  }
...
export const config = {
  matcher: '/web((?!/_next/static|/_next/image|/favicon.ico|/fonts|/assets).*)',
};
