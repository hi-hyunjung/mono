...
class client {
  ...
  constructor() {
    ...
    createAuthRefreshInterceptor(this.axiosInstance, async (failedRequest: { response: AxiosResponse }) => {
      if (failedRequest.response.status !== 401) return;
      try {
        const jwt = await cookieStorage.getItem('jwt');
        const { data } = await axios.get<Jwt>(`${env.NEXT_PUBLIC_API_BASE_URL}/api/admin/auth/refresh`, {
          headers: { Authorization: `Bearer ${jwt?.refreshToken}` },
        });

        await cookieStorage.setItem('jwt', data);

        failedRequest.response.config.headers.setAuthorization(`Bearer ${data.accessToken}`);
      } catch {
  ...
  >({
    ...
    query,
    options,
  }: { path: TPath } & (TPathParams extends undefined ? { pathParams?: TPathParams } : { pathParams: TPathParams }) &
  ...
  >({
    path,
    pathParams,
  }: { path: TPath } & (TPathParams extends undefined ? { pathParams?: undefined } : { pathParams: TPathParams })) {
    return this.axiosInstance.delete<TResponse>(getRequestUrl(path, pathParams));
  }
  ...
  } & (TPathParams extends undefined ? { pathParams?: undefined } : { pathParams: TPathParams }) &
    (TBody extends undefined ? { body?: undefined } : { body: TBody })) {
    return this.axiosInstance.post<TResponse>(getRequestUrl(path, pathParams), body, config);
  }
  ...
  >({
    ...
    (TBody extends undefined ? { body?: undefined } : { body: TBody })) {
    return this.axiosInstance.put<TResponse>(getRequestUrl(path, pathParams), body);
  }
  ...
  >({
    ...
    (TBody extends undefined ? { body?: undefined } : { body: TBody })) {
    return this.axiosInstance.patch<TResponse>(getRequestUrl(path, pathParams), body);
  }
}
